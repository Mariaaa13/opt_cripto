...
struct Punct
{
	int x;
	int y;
	bool EN;//EN - pentru elementul neutru 
};
struct Curba
{
	int a, b, p;
};
Punct aduna(Curba C, Punct P, Punct Q)
{
	if (P.EN)
		return Q;
	if (Q.EN)
		return P;
	Punct R;
	if (!modulo(P.x - Q.x, C.p) && !modulo(P.y + Q.y, C.p)) {
		R.EN = true;
		return R;
	}
	int l;
	if (modulo(P.x - Q.x, C.p))
		l = modulo(modulo(P.y - Q.y, C.p) * invers(P.x - Q.x, C.p), C.p);
	else
		l = modulo(modulo((3 * P.x * P.x + C.a), C.p) * invers(2 * P.y, C.p),
			C.p);

	R.EN = false;
	R.x = modulo(l * l - P.x - Q.x, C.p);
	R.y = modulo(l * (P.x - R.x) - P.y, C.p);
	return R;
}
void af(ofstream & out, int p, Punct P)
{
	if (P.EN == true)
		out << "O";
	else out << "(" << P.x << "," << P.y << ")" << endl;
}
Punct inmulteste(Curba C, int n, Punct P) {
	Punct R;
	R.EN = true;
	while (n > 0)
	{
...
void afisare(Curba C)
{
	ofstream eout("puncte.txt");
	long long i = 0;
	eout << C.a << " " << C.b << endl;
	eout << "punctul " << i++ << " O" << endl;
	for (int x = 0; x < C.p; x++)
		for (int y = 0; y < C.p; y++)
			if (modulo(y * y - x * x * x - C.a * x - C.b, C.p) == 0)
				eout << "punctul " << i++ << " " << x << " " << y << endl;
}
