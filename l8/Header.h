...
RSA_key generare_chei()
{
	...
	int p, q, n, phi, e, d;
	srand((int)time(NULL));
	p = da_prim(2, 1000, 2);//Generam numere astfel incat produsul a doua 
	asemenea numere sa nu depaseasca domeniul pentru variabile
		q = da_prim(2, 1000, p);
	n = p * q;
	phi = (p - 1) * (q - 1);
	e = 3 + rand() % (phi - 3);
...
void semnaturaRSA(ifstream& in, ofstream& out, RSA_key key1, RSA_key key2) {
	ofstream out2("intermediary.txt");
	if(key1.n>key2.n)
}